...
This remains a three-direction batting challenge, not a full cricket simulation. Timing sets scoring outcomes, running is automatic, Liam stays on strike, there are three wickets, and real-world metre measurements are compressed for visibility. The ground's layout now follows the user's aerial photographs, though the distances between its landmarks are estimates read off them rather than survey figures; character likeness still needs photo references. Gameplay checks are not evidence of coaching-grade biomechanics.
